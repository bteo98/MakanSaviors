								...
								<div class="md-layout md-small-size-100">
									<div v-if="!processing">
										...
										<div class="text md-layout-item text-description">
											...
											<div v-if="isDonor">
												<div v-on:click="userProfile" class="username">
													<small class="text-description">Savior's Name: </small>
													{{
											...
											<small class="text-description">Collection Location:</small>
											{{ data["location"].join(", ") }}<br />
											<small class="text-description">Quantity Available:</small>
											{{ data["quantity"] }}<br />
											...
											<div
												v-if="
													!isAvailable &&
														(userRequest == 'accepted' || (donorStatus == 'accepted' && isDonor))
												"
												class="contact-info"
											>
												<div v-if="!isDonor">
											...
											<badge
												...
												v-if="!isAvailable && userRequest == null && !expired && !isDonor"
												>unavailable</badge
											>
											<badge
											...
											<badge
												...
												v-if="
													!isAvailable &&
														!expired &&
														(userRequest == 'declined' || (donorStatus == 'declined' && isDonor))
												"
												>declined</badge
											>
											<md-button
												class="md-warning first-button cancel-status"
												v-if="!isAvailable && !expired && userRequest === 'pending' && !isDonor"
										...
										<div class="text md-layout-item diet-restriction text-description">
											...
											<ul
												v-if="!processing && data.dietaryRestrictions.length > 0"
												class="md-layout"
												id="restriction-list"
											>
												<li
...
export default {
	...
	methods: {
		fetchImgData(storage) {
			...
			let imgPath = storage.ref(this.donorID + "/donationImages/" + this.foodID);
		...
		fetchFoodData: function(db) {
			db.collection("donationData")
				...
				.onSnapshot((doc) => {
					...
					this.data["expiry"] = new Date(doc.expiry.toDate().toLocaleString("en-US"));
		...
		updateStatus(statusMsg) {
			...
			if (statusMsg == "accepted") {
				...
				saviorDoc.get().then((doc) => {
					...
					requestDates[date] = date in requestDates ? requestDates[date] + 1 : 1;
					this.data.foodCategory.forEach((cat) => {
						request[cat] += 1;
					});
