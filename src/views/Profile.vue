		...
		<div class="main main-raised">
			<div class="section profile-content">
				<div class="container">
					...
					<div class="md-layout">
						<div class="text md-layout-item diet-restriction text-description ml-auto">
							<h4 style="font-size: 20px; text-align: center">
								Food Categories
							</h4>
							<ul class="md-layout" id="restriction-list">
								<li class="md-layout" v-for="categories in this.foodCategory" :key="categories">
									<badge class="md-layout-item profile-badges" type="info restrict">{{
										categories
									}}</badge>
								</li>
							</ul>
						</div>
						<div class="text md-layout-item diet-restriction text-description mr-auto">
							<h4 style="font-size: 20px; text-align: center">
...
>>>>>>> c41b0a689c7315edfeba97134250bce4d12bdfcf
</template>
	...
	methods: {
		...
		getDonationDetails: function() {
			for (let i = 0; i < this.imageIDs.length; i++) {
				var storageRef = firebase.storage().ref(this.UID + "/donationImages/" + this.imageIDs[i]);
				storageRef.getDownloadURL().then((url) => {
					var donationDict = {};
    ...
    getUserData: function() {
      database
        ...
        .then(doc => {
          ...
          this.numRatings = data.numRatings;
          this.joinDate = this.getDate(data.joinDate);
          this.avgRatings = this.getAvgRatings(
            this.totalRatings,
            this.numRatings
          );
          this.getImageIDs();
