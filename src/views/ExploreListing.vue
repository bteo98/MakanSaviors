			...
			<div class="section">
				<div class="container">
											...
											<div class="md-list-item-content">
												<drop-down direction="down">
													...
													<ul class="dropdown-menu dropdown-with-icons">
														<li>
															<a href="javascript:void(0)" v-on:click="filter(false, 'expiry')">
														...
														<li>
															<a href="javascript:void(0)" v-on:click="filter(false, 'datePosted')">
																<i class="material-icons">today</i>
																<p>
																	Date posted
																</p>
					...
					<div class="progress-pagination">
						<div class="md-layout">
							<div class="md-layout-item md-size-15 md-xsmall-size-30 condition">
								...
								<div class="flex-column">
									...
									>
									<md-checkbox value="Contains Peanuts" v-model="dietaryNotFilter"
										>No Peanuts</md-checkbox
									>
									<md-checkbox value="Contains Shellfish" v-model="dietaryNotFilter"
								...
								<div class="flex-column">
									<md-checkbox value="Bento Boxes" v-model="foodCatFilter">Bento Boxes</md-checkbox>
									<md-checkbox value="Canned Food" v-model="foodCatFilter">Canned Food</md-checkbox>
									<md-checkbox value="Dairy" v-model="foodCatFilter">Dairy</md-checkbox>
									<md-checkbox value="Drinks" v-model="foodCatFilter">Drinks</md-checkbox>
									<md-checkbox value="Proteins" v-model="foodCatFilter">Proteins</md-checkbox>
									<md-checkbox value="Carbohydrates" v-model="foodCatFilter"
										>Carbohydrates</md-checkbox
									>
									<md-checkbox value="Vegetables and Fruits" v-model="foodCatFilter"
										>Vegetables / Fruits</md-checkbox
									>
									<md-checkbox value="Snacks" v-model="foodCatFilter">Snacks</md-checkbox>
								</div>
								<br />
								<md-button class="md-raised md-success" v-on:click="filter(false, orderby)"
									>Filter</md-button
								>
							</div>
							<br />
							<div class="item-listing">
								<ul id="itemsList" v-if="!processing">
									<li class="md-layout explore-card" v-for="item in collections" :key="item.foodID">
...
export default {
	...
	methods: {
			...
			db.collection("donationData")
				...
				.onSnapshot((snapshot) => {
					...
					snapshot.forEach((doc) => {
						...
						data["datePosted"] = new Date(doc.datePosted.toDate().toLocaleString("en-US"));
		...
		filter(reverse, orderby) {
			...
			var collect = db.collection("donationData").where("status", "==", "available");
			...
			if (this.foodCatFilter.length != 0) {
				collect = collect.where("foodCategory", "array-contains-any", this.foodCatFilter);
			}
			if (reverse) {
			...
			collect.orderBy(orderby, this.order).onSnapshot((snapshot) => {
				...
				snapshot.forEach((doc) => {
					...
					if (data.donorID != this.user && data.expiry >= new Date() && restrictCond) {
						this.collections.push(data);
					}
				});
